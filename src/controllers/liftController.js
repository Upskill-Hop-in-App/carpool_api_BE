...
class LiftController {
  async createLift(req, res) {
    ...
    try {
      ...
      logger.error("LiftController - Error creating lift");
  ...
  async getAllLifts(req, res) {
    ...
    try {
      ...
      logger.error("LiftController - Failed to retrieve lifts", err);
  ...
  async getLiftByCode(req, res) {
    ...
    try {
      ...
      logger.error("LiftController - Failed to retrieve lift by code");
      if (err.message === "LiftNotFound") {
  ...
  async updateLiftByCode(req, res) {
    ...
    try {
      ...
      logger.error("LiftController - Error updating lift");
      if (err.name === "ValidationError") {
        let errorMessage = `${MESSAGES.VALIDATION_ERROR}: `;
        for (let field in err.errors) {
          errorMessage += `${err.errors[field].message}`;
  ...
  async deleteLiftByCode(req, res) {
    ...
    try {
      ...
      logger.error("LiftController - Error deleting lift");
      if (err.message === "LiftNotFound") {
        res.status(400).json({ error: MESSAGES.LIFT_NOT_FOUND_BY_CODE });
